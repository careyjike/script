...
Install_MariaDB100() {
  ...
  if [ "${install_mod}" == "binary" ]; then
  	# download
  	src_url=https://mirrors.tuna.tsinghua.edu.cn/mariadb/mariadb-${mariadb100_version}/bintar-${GLIBC_FLAG}-${SYS_BIT_a}/mariadb-${mariadb100_version}-${GLIBC_FLAG}-${SYS_BIT_b}.tar.gz && wget -c --tries=6 --no-check-certificate $src_url
  	# src_url=https://downloads.mariadb.org/interstitial/mariadb-${mariadb100_version}/bintar-${GLIBC_FLAG}-${SYS_BIT_a}/mariadb-${mariadb100_version}-${GLIBC_FLAG}-${SYS_BIT_b}.tar.gz && wget -c --tries=6 --no-check-certificate $src_url
    ...
    sed -i "s@/usr/local/mysql@${mariadb_install_dir}@g" ${mariadb_install_dir}/bin/mysqld_safe
  	...
  	# download
  	src_url=https://mirrors.tuna.tsinghua.edu.cn/mariadb/mariadb-${mariadb100_version}/source/mariadb-${mariadb100_version}.tar.gz && wget -c --tries=6 --no-check-certificate $src_url
  	# src_url=https://downloads.mariadb.org/interstitial/mariadb-${mariadb100_version}/source/mariadb-${mariadb100_version}.tar.gz && wget -c --tries=6 --no-check-certificate $src_url
  ...
  if [ -d "${mariadb_install_dir}/support-files" ]; then
    echo -e "${CSUCCESSFUL}MariaDB installed successfully! ${CEND}"
    if [ "${install_mod}" == "binary" ]; then
    ...
    rm -rf mariadb-${mariadb100_version}
    echo -e "${CFAIL}MariaDB install failed, Please contact the author! ${CEND}"
    kill -9 $$
  fi

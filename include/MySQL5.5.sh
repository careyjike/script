...
Install_MySQL55() {
  ...
  if [ "$install_mod"  == 'binary' ]; then
    ...
    src_url=http://mirrors.sohu.com/mysql/MySQL-5.5/mysql-${mysql55_version}.tar.gz && wget --tries=6 -c $src_url
  ...
  [ ! -d "$mysql_data_dir" ] && mkdir -p ${mysql_data_dir};chown -R mysql.mysql $mysql_data_dir

  if [ "$install_mod" == 'binary' ]; then
  	tar xvf mysql-${mysql55_version}-linux2.6-${SYS_BIT}.tar.gz
    mv mysql-${mysql55_version}-linux2.6-${SYS_BIT}/* ${mysql_install_dir}
    sed -i 's@executing mysqld_safe@executing mysqld_safe\nexport LD_PRELOAD=/usr/local/lib/libjemalloc.so@' ${mysql_install_dir}/bin/mysqld_safe
  elif [ "$install_mod" == 'source' ]; then
    tar xvf mysql-${mysql55_version}.tar.gz
    pushd mysql-${mysql55_version}
    [ "${arm}" == "y" ] && patch -p1 < ../mysql-5.5-fix-arm-client_plugin.patch
...
EOF
  ...
  chown ${run_user}.${run_user} -R ${mysql_data_dir}

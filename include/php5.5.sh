...
Install_PHP55() {
  pushd ${Pwd}/src
  src_url=http://ftp.gnu.org/pub/gnu/libiconv/libiconv-${libiconv_version}.tar.gz && wget -c --tries=6 $src_url
  src_url=https://curl.haxx.se/download/curl-${curl_version}.tar.gz && wget -c --tries=6 $src_url
  src_url=http://downloads.sourceforge.net/project/mcrypt/Libmcrypt/${libmcrypt_version}/libmcrypt-${libmcrypt_version}.tar.gz && wget -c --tries=6 $src_url
  src_url=http://downloads.sourceforge.net/project/mhash/mhash/${mhash_version}/mhash-${mhash_version}.tar.gz && wget -c --tries=6 $src_url
  src_url=http://downloads.sourceforge.net/project/mcrypt/MCrypt/${mcrypt_version}/mcrypt-${mcrypt_version}.tar.gz && wget -c --tries=6 $src_url
  src_url=http://oss.akhack.com/src/php/libiconv-glibc-2.16.patch && wget -c --tries=6 $src_url
  src_url=http://www.php.net/distributions/php-${php55_version}.tar.gz && wget -c --tries=6 $src_url
  src_url=http://oss.akhack.com/src/php/fpm-race-condition.patch && wget -c --tries=6 $src_url

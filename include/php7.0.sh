...
Install_PHP70() {
  ...
  #src_url=${OSS_Url}/mhash/mhash-${mhash_version}.tar.gz && wget --tries=6 -c $src_url
  src_url=http://downloads.sourceforge.net/project/mcrypt/MCrypt/${mcrypt_version}/mcrypt-${mcrypt_version}.tar.gz && wget --tries=6 -c $src_url
  src_url=${OSS_Url}/mcrypt/mcrypt-${mcrypt_version}.tar.gz && wget --tries=6 -c $src_url
  src_url=${OSS_Url}/lib/libiconv-glibc-2.16.patch && wget --tries=6 -c $src_url
  src_url=http://www.php.net/distributions/php-${php70_version}.tar.gz && wget --tries=6 -c $src_url
  ...
  if [[ $Apache_is_install =~ ^1$ ]] || [ -e "$apache_install_dir/bin/apxs" ]; then
